...
#         for i in range(len(self._feat_stride)):
#             # 1. Generate proposals from bbox deltas and shifted anchors
#             shift_x = np.arange(0, w[i]) * self._feat_stride[i]
#             shift_y = np.arange(0, h[i]) * self._feat_stride[i]
#             shift_x, shift_y = np.meshgrid(shift_x, shift_y)
...
#             A =  i_anchors.shape[0]
#             K = shifts.shape[0]
#             all_anchors_ = (i_anchors.reshape((1, A, 4)) +
#                         shifts.reshape((1, K, 4)).transpose((1, 0, 2)))
#             all_anchors_ = all_anchors_.reshape((K * A, 4))
#             total_anchors = total_anchors + int(K * A)
        
#             all_anchors_list.append(all_anchors_)
# ################concat all anchors

#         all_anchors = all_anchors_list[0]
#         for i in range(1,len(self._feat_stride)):
#             all_anchors = np.vstack((all_anchors, all_anchors_list[i]))
#             # only keep anchors inside the image
...
#         # labels
#         s =  total_anchors/A
#         labels = labels.reshape((1, s, A)).transpose(0, 2, 1)
#         labels = labels.reshape((1, 1, A * s))
...
#             .reshape((1, s, A * 4)).transpose(0, 2, 1)
#         top[1].reshape(*bbox_targets.shape)
#         top[1].data[...] = bbox_targets
